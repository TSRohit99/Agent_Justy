...
async function generateImage(values, flag) {
  try {
    ...
    const baseImage = await loadImage('./lib/baseImage.png');
    // const baseImage = await loadImage('baseImage.png');
    ...
    if (flag === "lucky") {
      ...
      if(values.game.length>17){
        ctx.font = "bold 25px Arial";
      }

      let xAxis = 785;
      if(values.game.length<9){
        xAxis = 880;
      }
      
      ctx.fillText(values.game, xAxis, 110);
    ...
    // Draw the dollar value
    if(values.volume<1000000){
    ctx.fillText(formatValue(values.volume), 470, 355);
    }
    else{
      ctx.fillText(parseInt(values.volume), 470, 355);
    }
    }
...
// generateImage({multiplier: 2.5, level: 3, game: "Roulette", profit: 1000, player:"0xjsjhdfjhdsjhdgdsgs"}, "lucky");
// generateImage({won: 1000000, bet: 1000, volume: 1440753, player:"0xjsjhdfjhdsjhdgdsgs"}, "highRoller");
