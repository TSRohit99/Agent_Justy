...
Keep the wins rolling! #LuckyWinner #JustBet
    ...
    //get Image buffer
    const buffer = await generateImage(res, "lucky");
...
Keep crushing it! 🏆 #HighRoller #JustBet
    `;
    //get Image buffer
    const buffer = await generateImage(res, "highRoller");
...
Keep soaring! 🚀 #TopGainer #JustBet
    `;
    //get Image buffer
    const buffer = await generateImage(res, "gainer");
...
const executeSequentially = async () => {
  try {
    console.log("Starting daily tweets execution at:", new Date().toISOString());
    ...
    console.log("All tweets completed successfully at:", new Date().toISOString());
  } catch (error) {
